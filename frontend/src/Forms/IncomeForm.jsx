...
export default function IncomeForm() {
  const { register,handleSubmit,watch,formState: { errors },} = useForm({ defaultValues: { source: "Salary", amount: 0, date: new Date().toISOString().split('T')[0] } });
    const {source, amount, date} = watch();
  ...
  return (
    ...
    <div className="flex bg-gray-100 justify-center w-screen h-screen items-start">
        <form className="bg-gray-200 p-10 m-5 w-100" onSubmit={handleSubmit(SubmitHandler)}>
            <h1 className="mb-4">Insert income Data.</h1>
            <div className="mb-1 p-2"> Source</div>
            <input className="mb-2 rounded-md bg-white p-2" defaultValue="Salary" {...register("source")} />
